...
def dl_all_game_icon():
    ...
    total = len(all_game_id_list)
    for idx, app_id in enumerate(all_game_id_list, 1):
        product_info = client.get_product_info(apps=[app_id, ])
        if app_id not in product_info.get('apps', {}):
            print(f"[{idx}/{total}] Skipping app {app_id}: no product info available")
            continue
        app_data = product_info['apps'][app_id]
        if 'common' not in app_data:
            print(f"[{idx}/{total}] Skipping app {app_id}: no common info available")
            continue
        game_info = app_data['common']
        try:
        ...
        if "clienticon" in list(game_info.keys()):
            if os.path.exists(os.path.join(steam_icon_folder, game_info["clienticon"] + '.ico')):
                print(f"[{idx}/{total}] " + steam_game_icon_existed_notice[display_lang] % game_name)
                continue
            ...
            while trials < max_retry:
                try:
                    ...
                    trials += 1
                    print(f"[{idx}/{total}] " + steam_game_icon_dlretry_notice[display_lang] % (game_name, trials, max_retry))
            if os.path.exists(game_icon_filename):
                print(f"[{idx}/{total}] " + steam_game_icon_dlsuccess_notice[display_lang] % game_name)
            else:
                print(f"[{idx}/{total}] " + steam_game_icon_dlfail_notice[display_lang] % game_name)
        else:
            print(f"[{idx}/{total}] " + steam_game_icon_dlna_notice[display_lang] % game_name)
    return True
